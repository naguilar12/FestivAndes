--RF9
EXPLAIN PLAN SET STATEMENT_ID = 'EX_RF9' FOR WITH LISTA_FUNCIONES AS (SELECT IDFUN FROM 
(SELECT ID AS IDFUN FROM FUNCION F INNER JOIN 
(SELECT ID AS IDESPECT FROM ESPECTACULO E INNER JOIN (
(SELECT CE.ID_ESPECTACULO FROM COMPANIA_ESPECTACULO CE WHERE CE.ID_COMPANIA = 2))
...
NATURAL JOIN USUARIO U WHERE ID !=99 order by nombre;

--RF10
EXPLAIN PLAN SET STATEMENT_ID = 'EX_RF10' FOR WITH LISTA_FUNCIONES AS (SELECT IDFUN FROM (SELECT ID AS IDFUN FROM FUNCION F 
...
NATURAL JOIN CLIENTE NATURAL JOIN USUARIO WHERE ID!=99 order by '';
