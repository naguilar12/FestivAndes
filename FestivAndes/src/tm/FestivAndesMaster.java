...
public class FestivAndesMaster {
	...
	{
		DAOTablaCliente daoCliente = new DAOTablaCliente();
		DAOTablaCompa駃a daoCompania = new DAOTablaCompa駃a();
		...
		try
		{
			...
			conn.setSavepoint();
			Compa駃aTeatro compa駃a = daoCompania.darInfoCompaniasId(idComp);
			List<Espectaculo> espectaculosCompa駃a= compa駃a.getEspectaculos().getEspectaculos();
			List<Funcion> todasFuncionesCompa駃a = new ArrayList<>();
			for (Espectaculo espectaculo : espectaculosCompa駃a) 
			{
				for (Funcion funcion : espectaculo.getFunciones()) 
				{					
					todasFuncionesCompa駃a.add(funcion);
				}
			}
			...
			for (int i = 0; i < boletas.size(); i++) 
			{
				...
				for (int j = 0; j < todasFuncionesCompa駃a.size() && !encontro; j++) 
				{
					Funcion actualFun = todasFuncionesCompa駃a.get(j);
		...
		try
		{
			...
			for (int i = 0; i < todosClientes.size(); i++) 
			{
				...
				for (int j = 0; j < funcionesCliente.size(); j++) 
				{
					...
					for (int k = 0; k < todasFuncionesCompa駃a.size() && !encontrado; k++) 
					{
						if(actualFun.getFuncion().getId()==todasFuncionesCompa駃a.get(k).getId())
						{
							encontrado= true;
						}
					}
