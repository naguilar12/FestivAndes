...
public class FestivAndesMaster {
	...
	{
		DAOTablaFuncion daoFuncion = new DAOTablaFuncion();
		DAOTablaCompa駃a daoTablaCompa駃a = new DAOTablaCompa駃a();
		...
		try 
		{
			...
			conn.setTransactionIsolation(Connection.TRANSACTION_SERIALIZABLE);
			daoFuncion.setConn(conn);
			daoTablaCompa駃a.setConn(conn);
			conn.setSavepoint();
			List<NotaDebito> nueva = new ArrayList<NotaDebito>();
			Compa駃aTeatro compa = daoTablaCompa駃a.darInfoCompaniasId(idC);
			...
			daoTablaCompa駃a.cancelarCompania(compa.getId());
			conn.commit();
			conn.setAutoCommit(true);	
		}
		catch (SQLException e) {
