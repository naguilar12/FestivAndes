...
public class FestivAndesClienteServices {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response consultarNoAsistenciaAlFestival(RFC1 info) {
		FestivAndesMaster tm = new FestivAndesMaster(getPath());
		ArrayList<Funcion> resultado;
		try {
			resultado = tm.darFunciones(info.getCompania(), info.getFechaInicio(), info.getFechaFin(), info.getAgrupamiento(), info.getIdioma(),info.getTraduccion());
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(resultado).build();
