...
public class FestivAndesOrganizadoresServices {
	...
	public Response cancelarFuncion(@javax.ws.rs.PathParam("idO") int idO,@javax.ws.rs.PathParam("idF") int idF) {
		...
		try {
			if(tm.darOrganizador(idO)!=null)
				resultado = tm.cancelarFuncion(idF, true);
			else
				resultado = null;
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
	...
	@GET
	@Path("retirarCompania/{idC}")
	@Produces({ MediaType.APPLICATION_JSON })
	public Response retirarCompania(@javax.ws.rs.PathParam("idC") int idC) {
		FestivAndesMaster tm = new FestivAndesMaster(getPath());
		List<NotaDebito> resultado;
		try {
				resultado = tm.retirarCompania(idC);
		} catch (Exception e) {
			return Response.status(500).entity(doErrorMessage(e)).build();
		}
		return Response.status(200).entity(resultado).build();
